...
function getRiskRecommendations(network: Network) {
  if (network.auth === 'Open' || network.risk === 'Dangerous' || network.risk === 'Critical') {
    return {
      icon: AlertTriangle,
      color: 'text-[#dc2626]',
      bgColor: 'bg-[#fee2e2]',
      title: 'High Risk Network',
      description: 'This network has weak or unsafe security configuration.',
      recommendations: [
  ...
  if (network.auth === 'WPA' || network.encryption === 'TKIP' || network.risk === 'Weak') {
    return {
      icon: AlertTriangle,
      color: 'text-[#dc2626]',
      bgColor: 'bg-[#fee2e2]',
      title: 'High Risk - Outdated Security',
      description: 'This network uses outdated security protocols.',
      recommendations: [
  ...
  if (network.auth.includes('WPA2')) {
    return {
      icon: Shield,
      color: 'text-[#eab308]',
      bgColor: 'bg-[#fef9c3]',
      title: 'Medium Risk - Standard Security',
      description: 'This network uses WPA2 security.',
      recommendations: [
  ...
  if (network.auth.includes('WPA3')) {
    return {
      icon: CheckCircle2,
      color: 'text-[#16a34a]',
      bgColor: 'bg-[#dcfce7]',
      title: 'Low Risk - Strong Security',
      description: 'This network uses modern security protocols.',
      recommendations: [
...
export function NetworkDetailsModal({ network, onClose }: NetworkDetailsModalProps) {
  ...
  const signalQuality =
    network.signal >= 80 ? 'Excellent' :
    network.signal >= 60 ? 'Good' :
    network.signal >= 40 ? 'Fair' :
    'Weak';

  const signalPercentage = Math.min(100, Math.max(0, network.signal));
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={onClose}>
      <div className="bg-white rounded-xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 space-y-6">
          <div className={`${riskInfo.bgColor} rounded-xl p-6 border-2`}>
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-medium text-[#1f2937] mb-2">Security Recommendations:</p>
                <ul className="space-y-2">
                  {riskInfo.recommendations.map((rec, index) => (
                    <li key={index} className="flex items-start gap-2 text-sm text-[#1f2937]">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#0c7c84] mt-1.5" />
                      {rec}
                    </li>
                  ))}
                </ul>
          ...
          <div className="bg-white rounded-xl border border-[#e5e7eb] p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-[#1f2937]">{network.signal}%</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-[#e5e7eb] p-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-[#1f2937]">{signalPercentage}%</span>
              </div>
        ...
        <div className="sticky bottom-0 bg-[#fafbfc] border-t border-[#e5e7eb] p-6">
          <button
            onClick={onClose}
            className="w-full px-6 py-3 bg-[#0c7c84] text-white rounded-lg font-medium hover:bg-[#0a6d73]"
          >
            Close
          </button>
        </div>
